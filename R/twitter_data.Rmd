...
(am + pm) +
  labs(title = "Retweeter Locations (excluding UK)") +
...
jr_hashtags %>% 
  ...
  scale_fill_brewer(palette = "Dark2", 
                    name = "Hashtag Used",
                    labels = c("No", "Yes")) +
  labs(title = "Impact of hashtags on @jumping_uk tweets") +
  xlab("Hashtag") +
  ylab("Average Interactions") +
  theme_classic() +
...
jr = c("csgillespie", "nrennie35", "StatsRhian", 
       ...
       "jwalton3141", "parissagregg", "haematobot",
       "_jcken")

retweeters %<>%
  mutate(jumping_rivers = FALSE)
retweeters %<>%
  select(user_id, screen_name, followers_count, jumping_rivers)

for(i in 1:nrow(retweeters)){
  if(retweeters$screen_name[i] %in% jr){
    retweeters$jumping_rivers[i] = TRUE
  }
}
```
