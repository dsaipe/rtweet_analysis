...
rstats %<>%
  select(screen_name, followers_count, created_at, text, favorite_count, retweet_count, hashtags, media_type, urls_url, mentions_screen_name) %>%
  mutate(hashtag_num = str_count(hashtags, "\\w+"),
         interactions = retweet_count + favorite_count,
         hour_posted = hour(created_at))
rstats$text = str_remove_all(rstats$text, "#[A-z0-9]+") %>%
  str_squish()
```
...
```{r common word}
tweet_words = unnest_tokens(tbl = rstats,
                            output = word,
                            input = text)
tweet_words = anti_join(tweet_words, stop_words,
                        by = "word")
custom_stop_words = c("t.co", "https")
words_count = count(tweet_words, word, sort = TRUE)
words_count %<>%
