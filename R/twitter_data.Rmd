---
title: '[@jumping_uk](https://twitter.com/jumping_uk) Twitter Analysis'
author: "David Saipe"
date: 'July 26, 2022'
output: 
  pdf_document:
...
The [@jumping_uk](https://twitter.com/jumping_uk) account has been active since March 2016 and has almost
...
data from the [@jumping_uk](https://twitter.com/jumping_uk) timeline, as well as general tweets that use
'#rstats'. 
...
There is mixed opinion about whether tweets posted on the weekend receive as much interaction as those posted during in the week. The best day to post a tweet usually depends on the type of content that is being posted, and the account it is being posted from. For example, [one article](https://recurpost.com/blog/best-time-to-tweet/) suggests that the best time to post about consumer goods is on the weekend, whereas the best time to post for technical business is during the week. [@jumping_uk](https://twitter.com/jumping_uk)’s timeline corroborates with this idea as tweets that are posted on Saturdays and Sundays usually receive less retweets and likes, compared to tweets posted during the week.
...
ggplot(by_weekday, aes(day, interactions)) +
  ...
  labs(caption = "Taken from [@jumping_uk](https://twitter.com/jumping_uk) timeline from 2016 to 2022") +
  xlab("Day") +
...
From looking at the [@jumping_uk](https://twitter.com/jumping_uk) timeline, tweets posted between 8-11am have generally received the highest number of interactions. There are however spikes in engagement from 2pm onwards. 

```{r time posted, echo = FALSE}
times = jr_timeline %>%
  filter(!(year == 2016))

hour_count = ggplot(times, aes(hour_posted)) + 
...
hour_interactions = ggplot(avg_interactions, aes(hour_posted, mean)) +
...
hour_count + hour_interactions
```

In recent years, the [@jumping_uk](https://twitter.com/jumping_uk) has been posting much more frequently in the morning, yet the accounts top three highest interacted with tweets have all come in the afternoon. Examining the locations of people retweeting [@jumping_uk](https://twitter.com/jumping_uk) tweets, there
is a higher proportion of retweeters in the afternoon peak that are from
America (North, South and Central) with a lower proportion being from the UK, Africa and Asia, because of the time zone difference. 
...
am +
  labs(title = "Locations of [@jumping_uk](https://twitter.com/jumping_uk) retweeters") + 
  pm
```
...
It should also 
be taken into account that there are more [@rstatstweet](https://twitter.com/rstatstweet) followers from America than anywhere else, 
so tweets containing #rstats may receive more engagement in the afternoon.

```{r rstats follower locations, echo = FALSE, fig.width = 4.333, fig.height = 3, fig.align = 'center'}
...
ggplot(rstats_followers, 
  ...
  coord_polar("y", start = 0) +
  scale_fill_brewer(palette = "Dark2", name = "Continent") +
  labs(title = "Locations of [@rstatstweet](https://twitter.com/rstatstweet) followers",
...
Images do affect levels of engagement positively. The figure below shows the average levels of interaction for tweets with no media and tweets that include a photo, taken from both the #rstats timeline and the [@jumping_uk](https://twitter.com/jumping_uk) timeline.
...
avg_interactions %>%
  ...
  scale_fill_brewer(palette = "Dark2", 
                    name = "Source",
                    labels = c("[@jumping_uk](https://twitter.com/jumping_uk)", "#rstats")) +
  theme_classic()
```
...
looking. Images can also be used to include information that can't be
...
There are multiple bots on twitter that retweet tweets containing #rstats ([@rstatstweet](https://twitter.com/rstatstweet) and [@rstats4ds](https://twitter.com/rstats4ds) are the two main ones). This hashtag should always be included in [@jumping_uk](https://twitter.com/jumping_uk) tweets. Below are the top 30 most commonly used hashtags alongside #rstats, taken from the 100 most popular tweets containing #rstats (07/07 - 14/07). 
...
Other than #rstats, [@jumping_uk](https://twitter.com/jumping_uk) uses #datascience, #python, #tidyverse
...
```{r jr hashtags, echo = FALSE, fig.width = 6, fig.height = 4.15}
jr_hashtags = tibble(
...
jr_hashtags %>% 
  ...
  labs(title = "Impact of hashtags used alongside #rstats",
       caption = "Taken from [@jumping_uk](https://twitter.com/jumping_uk) timeline from 2016 to 2022") +
  xlab("Hashtag") +
  ylab("Average Interactions") +
  theme_classic() +
...
[@jumping_uk](https://twitter.com/jumping_uk) doesn’t use #machinelearning and #bigdata as often; these are two hashtags that are often found in the most popular tweets on the #rstats timeline. The figure below shows that these have a significant impact on engagement on #rstats tweets, and so they could be used more often in the future.

```{r rstats hashtags, echo = FALSE, fig.width = 6, fig.height = 4.15}
...
The factors analysed in this report have an effect on the levels of engagement with [@jumping_uk](https://twitter.com/jumping_uk) tweets to varying extents.

There are indeed optimum times of the day to post on twitter. However, because of the difference in audience during the morning and afternoon, the best thing would be to post twice a day. Tweets can be tailored this way to match the receiving audience, with afternoon tweets being targeted towards American users, and morning tweets being targeted towards users in Asia and Africa.

It was already apparent that images were increasing the numbers of likes and retweets on [@jumping_uk](https://twitter.com/jumping_uk) posts, and this analysis has helped confirm that. It is always appropriate to include images in tweets, and when a URL is included within the body of a tweet, it is important to ensure that a summary card appears when the tweet is posted.

[@jumping_uk](https://twitter.com/jumping_uk) are already using some of the main hashtags used alongside #rstats in popular tweets. There are some hashtags which could be used more, such as #bigdata, #machinelearning, #programming, and #datascientist. It would also be worth following some of the #rstats bots, as they don’t always retweet #rstats tweets.
