...
most tweets in one year. The graph below shows how many tweets
@jumping_uk have posted each year since joining twitter.
...
There is mixed opinion about whether tweets posted on the weekend will
...
posted, and account it is being posted from. For example, one article
suggests that the best time to post about consumer goods is on the
weekend, whereas the best time to post for technical business is during
the week. @jumping_uk's timeline corroborates with this idea as tweets
that have been posted on Saturdays and Sundays usually receive less
...
engagement: one from 8-11am, and another from 3-6pm, as shown below.
...
ggplot(avg_interactions, aes(hour_posted, mean)) +
  geom_bar(fill = "#d95f02",  color = "black", stat = "identity") +
...
proportion of retweeters from the UK remains similar during both peaks
(just under 50%).
...
am = ggplot(morning_retweeters, 
  ...
  labs(subtitle = "Morning peak: 8am - 11am") +
  theme_void() +
...
simply by what time they are posted. For example, tweets about the
...
It should also be taken into account that a large proportion of
@rstatstweet followers are from America, so the impact of using #rstats
will be more significant in the afternoon.
...
Images do affect levels of engagement positively. The
figure below shows the average levels of interaction for tweets with no
media and tweets that include a photo, taken from both the #rstats
timeline and the @jumping_uk timeline.
...
There are multiple bots on twitter that retweet tweets containing #rstats 
(@rstatstweet and @rstats4ds are the two main ones). This
...
are two hashtags that are often found in the most popular tweets on the
#rstats timeline. The figure below shows that these have a significant
...
@jumping_uk are already using some of the main hashtags used alongside #rstats in popular tweets. The word cloud shows which hashtags have been used by @jumping_uk more than others.
