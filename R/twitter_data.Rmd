...
for(i in 1:nrow(jr_timeline)){
  ...
  hour = gsub("^0+", "", as.character(hour))
  jr_timeline = jr_timeline %>%
    mutate(hour_posted = strtoi(hour), interactions = retweet_count + favorite_count)
}
```

```{r time posted, include = FALSE}
ggplot(jr_timeline) +
  geom_bar(aes(hour_posted)) +
  xlab("Hour Posted (24 hr)") +
  ylab("Frequency")

ggplot(jr_timeline) +
  geom_smooth(aes(hour_posted, interactions))

```

```{r retweeters, include = FALSE}
status_id = c(jr_timeline$status_id)
retweeters = c(get_retweeters("1544677140653809664
")$user_id)
for(i in 2:69){
  retweeters = append(retweeters, get_retweeters(status_id[i])$user_id)
}

retweeters_data = read.csv("C:/Users/dfs20/OneDrive - Newcastle University/Stage 2/Jumping Rivers/rtweet_analysis/Data/retweeters_list.csv")
